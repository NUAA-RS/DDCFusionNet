...
class ConvEdgeDetector(nn.Module):
    def __init__(self, channels, kernel_size=3, padding=1):
        super(ConvEdgeDetector, self).__init__()
    ...
    def init_weights(self):
        ...
        sobel_x = sobel_x.unsqueeze(0).unsqueeze(0).repeat(self.conv.out_channels, 1, 1, 1)
        sobel_y = sobel_y.unsqueeze(0).unsqueeze(0).repeat(self.conv.out_channels, 1, 1, 1)
        self.conv.weight.data = torch.cat((sobel_x, sobel_y), dim=1)
        self.conv.bias.data.zero_()

    def forward(self, x):
        # edge_x = self.conv(x[:, :self.conv.out_channels//2, :, :])
        # edge_y = self.conv(x[:, self.conv.out_channels//2:, :, :])
        edge_x = self.conv(x[:, :2, :, :])
        edge_y = self.conv(x[:, :2, :, :])
...
class DMRM(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.grad_ir = ConvEdgeDetector(out_channels)
        self.grad_vi = ConvEdgeDetector(out_channels)
